...
Util::TextFilter::TextFilter(const PSDFormat& aFormat): mShiftJisDecoder(), mIsShiftJis() {
...
Util::createTextureImage(const PSDFormat::Header& aHeader, const PSDFormat::Layer& aLayer) {
    ...
    if (aLayer.rect.width() <= 0 || aLayer.rect.height() <= 0) {
        ...
        return std::pair<XCMemBlock, QRect>(block, QRect(aLayer.rect.left(), aLayer.rect.top(), 1, 1));
    }
