...
namespace ffd {
    ...
    void BrushMode::renderQt(const core::RenderInfo& aInfo, QPainter& aPainter) {
        // draw brush
        {
            ...
            const Qt::PenStyle style = mParam.type == FFDParam::Type_Pencil ? Qt::SolidLine : Qt::DotLine;
    ...
    bool BrushMode::executeDrawTask(const QVector2D& aCenter, const QVector2D& aMove) {
        ...
        for (int i = 0; i < mTargets.size(); ++i) {
            ...
            // set task type
            task->setType(mParam.type == FFDParam::Type_Pencil ? Task::Type_Deformer : Task::Type_Eraser);
        ...
        // make commands
        {
            ...
            if (!mStatus.commandRef) {
                ...
                for (int i = 0; i < mTargets.size(); ++i) {
                    ...
                    notifier->event().pushTarget(*mTargets[i]->node, TimeKeyType_FFD, frame);
                }
                ...
                for (int i = 0; i < mTargets.size(); ++i) {
                    ...
                    if (mTargets[i]->keyOwner.owns()) {
                        mTargets[i]->keyOwner.pushOwnsKey(stack, *mTargets[i]->node->timeLine(), frame);
                    }
                }
