...
void MeshKey::Data::copyVerticesEdgesAndFaces(const Data& aRhs) {
    ...
    for (auto prevFace : aRhs.mFaces) {
        ...
        nextFace->rawInit(*edgeMap[prevFace->edge(0)], *edgeMap[prevFace->edge(1)], *edgeMap[prevFace->edge(2)]);
...
void MeshKey::Data::resetArrayedConnection(ArrayedConnectionList& aDest, const gl::Vector3* aPositions) const {
    ...
    for (auto vtx : mVertices) {
        ...
        for (auto edgeNode = vtx->edges(); edgeNode; edgeNode = edgeNode->next) {
            ...
            auto offset = aPositions[connectIndex] - gl::Vector3::make(vtx2->x, vtx2->y, 0.0f);
...
QJsonObject MeshKey::Data::serializeToJson() const {
    QJsonObject mesh;
    // Offset
    mesh["OriginOffsetX"] = mOriginOffset.x();
    mesh["OriginOffsetY"] = mOriginOffset.y();
    // Vertices
...
bool MeshKey::Data::deserializeFromJson(QJsonObject json) {
    ...
    // Edge
    int edgeCount = json["EdgeCount"].toInt();
    if (edgeCount == 0) {
        return false;
    }
    ...
    for (int i = 0; i < edgeCount; i++) {
        ...
        if (!xc_contains(v0, 0, vtxCount - 1) || !xc_contains(v1, 0, vtxCount - 1)) {
            return false;
        }
    ...
    // Faces
    int faceCount = json["FaceCount"].toInt();
    if (faceCount == 0) {
        return false;
    }
    ...
    for (int i = 0; i < faceCount; i++) {
        ...
        if (!xc_contains(e0, 0, edgeCount - 1) || !xc_contains(e1, 0, edgeCount - 1) ||
...
MeshKey::createTriangle(const QVector<QVector2D>& aPos, const QVector<MeshVtx*>& aRef, cmnd::Vector& aCommands) {
    ...
    // create face
    if (!face) {
        auto command = new MeshKeyUtil::CreateFace(mData.mFaces, *edge[0], *edge[1], *edge[2]);
        command->exec();
        aCommands.push(command);
...
cmnd::Vector MeshKey::createRemover(MeshVtx& aVtx) {
    MeshKey* key = this;
    MeshVtx* vtx = &aVtx;
    auto command = new cmnd::LambdaScalable([=](cmnd::Vector& aCommands) { key->listNeedToRemove(*vtx, aCommands); });
...
void MeshKey::listNeedToRemove(MeshVtx& aStartingVtx, cmnd::Vector& aCommands) {
    ...
    QVector<MeshVtx*> killVertices;
    {
        ...
        aCommands.push(new MeshKeyUtil::RemoveVtx(*this, mData.mVertices, aStartingVtx));
    }
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (!vtx->hasOtherParents(killEdges)) {
                if (!killVertices.contains(vtx)) {
                    ...
                    aCommands.push(new MeshKeyUtil::RemoveVtx(*this, mData.mVertices, *vtx));
                }
            }
        }
    }
...
cmnd::Vector MeshKey::createRemover(MeshFace& aFace) {
    MeshKey* key = this;
    MeshFace* face = &aFace;
    auto command = new cmnd::LambdaScalable([=](cmnd::Vector& aCommands) { key->listNeedToRemove(*face, aCommands); });
...
cmnd::Vector MeshKey::createSplitter(
    ...
    cmnd::Vector result;
    result.push(new TrianglePusher(*this, aFace, aEdge0, aPos0, aEdge1, aPos1, aTail));
    return result;
}
...
MeshKey::splitTriangle(MeshFace& aFace, MeshEdge& aEdgeSide, const QVector2D& aPosOnEdge, cmnd::Vector& aCommands) {
    ...
    for (auto oppoVtx : oppositeVertices) {
        ...
        auto faceCommand0 = new MeshKeyUtil::CreateFace(mData.mFaces, *centerEdge, *cornerEdge0, *flankEdge0);
        faceCommand0->exec();
        ...
        auto faceCommand1 = new MeshKeyUtil::CreateFace(mData.mFaces, *centerEdge, *cornerEdge1, *flankEdge1);
        faceCommand1->exec();
...
MeshVtx* MeshKey::splitTriangle(
    ...
    MeshVtx* flankVtx[2];
    for (int i = 0; i < 2; ++i) {
        flankVtx[i] = (commonVtx == edges[i]->vtx(0)) ? edges[i]->vtx(1) : edges[i]->vtx(0);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        // create flank edge
        {
            auto command = new MeshKeyUtil::CreateEdge(mData.mEdges, *centerVtx[i], *flankVtx[i]);
            command->exec();
        ...
        // create flank edge
        {
            auto command = new MeshKeyUtil::CreateEdge(mData.mEdges, *centerVtx[i], *commonVtx);
            command->exec();
    ...
    for (int i = 0; i < 2; ++i) {
        for (auto oppoVtx : oppositeVertices[i]) {
            ...
            // split edge
            auto edgeCommand = new MeshKeyUtil::CreateEdge(mData.mEdges, *centerVtx[i], *oppoVtx);
            edgeCommand->exec();
            auto centerEdge = edgeCommand->newEdge();
            aCommands.push(edgeCommand);
            ...
            XC_PTR_ASSERT(cornerEdge0);
            auto faceCommand0 = new MeshKeyUtil::CreateFace(mData.mFaces, *centerEdge, *cornerEdge0, *flankEdgeA[i]);
            faceCommand0->exec();
            aCommands.push(faceCommand0);
            ...
            XC_PTR_ASSERT(cornerEdge1);
            auto faceCommand1 = new MeshKeyUtil::CreateFace(mData.mFaces, *centerEdge, *cornerEdge1, *flankEdgeB[i]);
            faceCommand1->exec();
            aCommands.push(faceCommand1);
        }
    }
    ...
    // create center face
    {
        ...
        {
            auto command = new MeshKeyUtil::CreateEdge(mData.mEdges, *centerVtx[0], *centerVtx[1]);
            command->exec();
        ...
        {
            auto command = new MeshKeyUtil::CreateEdge(mData.mEdges, *centerVtx[0], *flankVtx[1]);
            command->exec();
        ...
        // create face A
        {
            auto command = new MeshKeyUtil::CreateFace(mData.mFaces, *flankEdgeB[0], *flankEdgeB[1], *centerEdge);
            command->exec();
            aCommands.push(command);
        }
        // create face B
        {
            auto command = new MeshKeyUtil::CreateFace(mData.mFaces, *centerEdge, *flankEdgeA[1], *appendEdge);
            command->exec();
            aCommands.push(command);
        }
        // create face C
        {
            auto command = new MeshKeyUtil::CreateFace(mData.mFaces, *flankEdgeA[0], *appendEdge, *anotherEdge);
            command->exec();
            aCommands.push(command);
        }
    }
